...
enum BootSequence {
    ...
    static func run(device: USBDeviceHandle, progress: ((String) -> Void)? = nil) -> Bool {
        ...
        // Strategy A: EP 0x04 init (quick prime)
        logger.info("Strategy A: EP 0x04 init...")
        progress?("Boot: priming EP 0x04...")
        device.writeControl(DisplayPadProtocol.initMessage)

        if pollForAck(device: device, duration: 2, start: start, progress: progress) {
            return true
        }

        // Strategy B: EP 0x04 + EP 0x02 init (quick prime)
        logger.info("Strategy B: EP 0x04 + EP 0x02 init...")
        progress?("Boot: priming EP 0x02...")
        device.writeControl(DisplayPadProtocol.initMessage)
        ...
        // Strategy C: SET_REPORT + re-send on ffaa, wait up to 40s
        logger.info("Strategy C: SET_REPORT + ffaa re-init...")
        progress?("Boot: waiting for device...")
        device.setReportControl(DisplayPadProtocol.initMessage)

        let strategyStart = Date()
        while Date().timeIntervalSince(strategyStart) < 40 {
